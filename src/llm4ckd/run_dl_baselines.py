...
from llm4ckd.dl_baselines import (
    dl_model_registry,
    make_dl_pipeline,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run low-data DL/tabular foundation baselines for LLM4CKD."
    )
    ...
    p.add_argument("--device", default="cpu", help="Device for TabPFN, e.g., cpu or cuda")
    return p.parse_args()
...
def main():
    ...
    for seed in config["seeds"]:
        ...
        for n_train in training_sizes:
            low_idx = sample_low_data_indices(
                y_train_all.values,
                n_samples=n_train,
                seed=seed,
            )
            ...
            models = dl_model_registry(seed=seed, device=args.device)

            for model_name, model in tqdm(
                models.items(),
                desc=f"DL seed={seed}, n={n_train}",
            ):
                try:
                    ...
                    print(
                        f"[WARN] Skipping {model_name} "
                        f"seed={seed} n={n_train}: {exc}"
                    )
                    continue
